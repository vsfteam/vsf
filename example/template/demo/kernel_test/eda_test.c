...
imp_vsf_peda(example_peda)
{
    ...
    switch (evt) {
        ...
        case VSF_EVT_RETURN:
            vsf_trace_info("Use time [%2d]", vsf_systimer_tick_to_ms(vsf_systimer_get_tick() - vsf_local.time));
            vsf_trace_info("\t\tReturn To Level[%2d]\r\n", vsf_local.level);
            vsf_eda_return();
...
int kernel_eda_test_main(int argc, char *argv[])
{
#else
