...
{
    vsf_log_info("%s: vid%04X pid%04X interface%d" VSFCFG_DEBUG_LINEEND,
...
{
    vsf_log_info("%s: remove interface" VSFCFG_DEBUG_LINEEND, ifs->drv->name);
}
            ...
            uint_fast8_t endpoint, uint_fast8_t type, uint_fast16_t size)
{
    ...
    endpoint &= 0x0F;
    return 1|   (size << 1)             /* 15-bit size */
            |   (endpoint << 16)        /* 4-bit endpoint */
            |   (type << 20)            /* 2-bit type */
            |   (dev->speed << 22)      /* 2-bit speed */
            |   (dev->devnum << 24)     /* 7-bit address */
            |   (direction << 31);      /* 1-bit direction */
}
...
vsf_usbh_dev_t *vsf_usbh_alloc_device(vsf_usbh_t *host)
{
    vsf_usbh_dev_t *dev;
    ...
    if (    (host->drv->alloc_device != NULL)
        &&  (host->drv->alloc_device(&host->use_as__vsf_usbh_hcd_t, dev) != VSF_ERR_NONE)) {
free_dev:
        VSF_USBH_FREE(dev);
        return NULL;
    }

    dev->devnum = vsf_bitmap_ffz(&host->device_bitmap, VSF_USBH_CFG_MAX_DEVICE);
    if (dev->devnum == 0) { goto free_dev; }
    vsf_bitmap_set(&host->device_bitmap, dev->devnum);
    return dev;
}

static void vsf_usbh_clean_device(vsf_usbh_t *host, vsf_usbh_dev_t *dev)
{
    ASSERT((host != NULL) && (dev != NULL));
    vsf_usbh_free_urb(host, &dev->ep0.urb);
    if (host->drv->free_device != NULL) {
        host->drv->free_device(&host->use_as__vsf_usbh_hcd_t, dev);
    }
    if (dev->ifs != NULL) {
...
vsf_err_t vsf_usbh_alloc_urb(vsf_usbh_t *host, vsf_usbh_urb_t *urb)
{
    vsf_usbh_eppipe_t pipe;

    ASSERT((host != NULL) && (urb != NULL) && urb->pipe.is_pipe);
    pipe = urb->pipe;
    urb->urb_hcd = host->drv->alloc_urb(&host->use_as__vsf_usbh_hcd_t);
    if (urb->urb_hcd != NULL) {
...
void vsf_usbh_free_urb(vsf_usbh_t *host, vsf_usbh_urb_t *urb)
{
    ASSERT(     (host != NULL) && (urb != NULL)
            &&  (urb->urb_hcd != NULL));

    if (!urb->pipe.is_pipe) {
        vsf_usbh_eppipe_t pipe = urb->urb_hcd->pipe;
        vsf_usbh_urb_free_buffer(urb);
        host->drv->free_urb(&host->use_as__vsf_usbh_hcd_t, urb->urb_hcd);
        urb->pipe = pipe;
    }
}
...
void *vsf_usbh_urb_get_buffer(vsf_usbh_urb_t *urb)
{
    ASSERT((urb != NULL) && !urb->pipe.is_pipe);
    void *buffer = urb->urb_hcd->buffer;
...
vsf_err_t vsf_usbh_urb_get_status(vsf_usbh_urb_t *urb)
{
    ASSERT((urb != NULL) && !urb->pipe.is_pipe);
    return urb->urb_hcd->status;
}

vsf_err_t vsf_usbh_urb_get_actual_length(vsf_usbh_urb_t *urb)
{
    ASSERT((urb != NULL) && !urb->pipe.is_pipe);
    return urb->urb_hcd->actual_length;
}

void vsf_usbh_remove_interface(vsf_usbh_t *host, vsf_usbh_dev_t *dev,
            vsf_usbh_ifs_t *ifs)
{
    ASSERT((host != NULL) && (dev != NULL) && (ifs != NULL));
    const vsf_usbh_class_drv_t *drv = ifs->drv;
    if (drv) {
        vsf_usbh_on_remove_interface(ifs);
        drv->disconnect(host, dev, ifs->param);
        ifs->drv = NULL;
        ifs->param = NULL;
        ifs->cur_alt = 0;
    }
...
static void vsf_usbh_free_parser(vsf_usbh_t *host)
{
    vsf_usbh_dev_parser_t *parser = host->parser;
    if (parser != NULL) {
...
void vsf_usbh_disconnect_device(vsf_usbh_t *host, vsf_usbh_dev_t *dev)
{
    vsf_usbh_dev_t *dev_child;

    ASSERT((host != NULL) && (dev != NULL));

    vsf_slist_t *list = &dev->dev_parent->children_list;
    while (list != NULL) {
        if (list->pnext == dev) {
            list->pnext = dev->child_node.pnext;
            break;
        }
        list = list->pnext;
    }
    ...
    if (dev->num_of_ifs && (dev->ifs != NULL)) {
        ...
        for (uint_fast8_t i = 0; i < dev->num_of_ifs; i++, ifs++) {
            vsf_usbh_remove_interface(host, dev, ifs);
        }
    }
    while (dev->children_list.pnext != NULL) {
        dev_child = dev->children_list.pnext;
        dev->children_list.pnext = dev_child->child_node.pnext;
        vsf_usbh_disconnect_device(host, dev_child);
    }
    vsf_usbh_clean_device(host, dev);

    if (dev->devnum != 0) {
        vsf_bitmap_clear(&host->device_bitmap, dev->devnum);
    }

    if (host->dev_new == dev) {
        vsf_usbh_free_parser(host);
        vsf_teda_fini(&host->teda);
        host->dev_new = NULL;
    }
    VSF_USBH_FREE(dev);
}
...
vsf_err_t vsf_usbh_submit_urb(vsf_usbh_t *host, vsf_usbh_urb_t *urb)
{
    vsf_usbh_hcd_urb_t *urb_hcd;

    ASSERT(     (host != NULL) && (urb != NULL) && (urb->urb_hcd != NULL)
            &&  !urb->pipe.is_pipe);

    urb_hcd = urb->urb_hcd;
    urb_hcd->eda_caller = vsf_eda_get_cur();
#if VSF_USBH_CFG_ENABLE_ROOT_HUB == ENABLED
    if (urb_hcd->dev_hcd == &host->dev_rh->use_as__vsf_usbh_hcd_dev_t) {
        return vsf_usbh_rh_submit_urb(&host->use_as__vsf_usbh_hcd_t, urb_hcd);
    } else
#endif
    {
        return host->drv->submit_urb(&host->use_as__vsf_usbh_hcd_t, urb_hcd);
    }
}

vsf_err_t vsf_usbh_relink_urb(vsf_usbh_t *host, vsf_usbh_urb_t *urb)
{
    ASSERT((host != NULL) && (urb != NULL) && !urb->pipe.is_pipe);
    return host->drv->relink_urb(&host->use_as__vsf_usbh_hcd_t, urb->urb_hcd);
}

vsf_err_t vsf_usbh_control_msg(vsf_usbh_t *host, vsf_usbh_dev_t *dev,
            struct usb_ctrlrequest_t *req)
{
    ASSERT((host != NULL) && (dev != NULL) && (req != NULL));
    vsf_usbh_urb_t *urb = &dev->ep0.urb;
    vsf_usbh_hcd_urb_t *urb_hcd = urb->urb_hcd;

    if (!dev->is_ep0_rdy) {
        urb->pipe.value = vsf_usbh_get_pipe(dev, 0, USB_ENDPOINT_XFER_CONTROL, 64);
        vsf_err_t err = vsf_usbh_alloc_urb(host, urb);
    ...
    return vsf_usbh_submit_urb(host, urb);
}

vsf_err_t vsf_usbh_set_address(vsf_usbh_t *host, vsf_usbh_dev_t *dev)
{
    ASSERT((host != NULL) && (dev != NULL));
    struct usb_ctrlrequest_t req = {
    ...
    return vsf_usbh_control_msg(host, dev, &req);
}
vsf_err_t vsf_usbh_get_descriptor(vsf_usbh_t *host, vsf_usbh_dev_t *dev,
...
vsf_err_t vsf_usbh_get_class_descriptor(vsf_usbh_t *host, vsf_usbh_dev_t *dev,
            uint_fast16_t ifnum, uint_fast8_t type, uint_fast8_t id,
            uint_fast16_t size)
{
    ASSERT((host != NULL) && (dev != NULL));
    struct usb_ctrlrequest_t req = {
...
vsf_err_t vsf_usbh_set_configuration(vsf_usbh_t *host, vsf_usbh_dev_t *dev,
...
vsf_err_t vsf_usbh_set_interface(vsf_usbh_t *host, vsf_usbh_dev_t *dev,
...
vsf_usbh_match_interface_driver(vsf_usbh_t *host, vsf_usbh_dev_parser_t *parser,
        vsf_usbh_ifs_parser_t *parser_ifs)
{
    vsf_usbh_ifs_t *ifs = parser_ifs->ifs;
    vsf_usbh_class_t *class = host->class_list.pnext;
...
static vsf_err_t vsf_usbh_find_intrface_driver(vsf_usbh_t *host,
        vsf_usbh_dev_parser_t *parser, vsf_usbh_ifs_parser_t *parser_ifs)
{
    vsf_usbh_ifs_t *ifs = parser_ifs->ifs;
    const vsf_usbh_class_drv_t *drv;
    ...
    do {
        drv = vsf_usbh_match_interface_driver(host, parser, parser_ifs);
        if (drv != NULL) {
            param = drv->probe(host, host->dev_new, parser_ifs);
            if (param != NULL) {
                ifs->param = param;
                ifs->drv = drv;
                if (VSF_ERR_NONE != vsf_usbh_on_match_interface(parser, parser_ifs)) {
                    drv->disconnect(host, host->dev_new, param);
                    continue;
                }
                return VSF_ERR_NONE;
            }
...
static vsf_err_t vsf_usbh_parse_config(vsf_usbh_t *host, vsf_usbh_dev_parser_t *parser)
{
    vsf_usbh_dev_t *dev = host->dev_new;
    ...
    while ((size > 0) && (size >= desc_header->bLength)) {
        ...
        case USB_DT_INTERFACE:
            ...
            if (!parser_ifs->parser_alt) {
                if (desc_ifs->bInterfaceNumber == ifnum) {
                ...
                probe_alt:
                    ...
                    claiming = size > 0 ? 1 : 0;
                    ifnum++;
                    if (ifnum < dev->num_of_ifs) {
                        parser_ifs[1].ifs = parser_ifs[0].ifs++;
                        parser_ifs++;
                    }
                    tmpsize = size;
                    header_tmp = desc_header;
    ...
    vsf_usbh_on_dev_parsed(host->dev_new, parser);

    // probe
    claiming = 0;
    parser_ifs = parser->parser_ifs;
    for (ifnum = 0; ifnum < dev->num_of_ifs; ifnum++, parser_ifs++) {
        if (!vsf_usbh_find_intrface_driver(host, parser, parser_ifs)) {
            claiming++;
        }
    }
...
{
    vsf_usbh_t *host = container_of(eda, vsf_usbh_t, teda);
    ...
    case VSF_EVT_MESSAGE:
        ...
        case VSF_USBH_PROBE_WAIT_DEVICE_DESC:
            buffer = vsf_usbh_urb_get_buffer(urb);
        ...
        case VSF_USBH_PROBE_WAIT_FULL_DEVICE_DESC:
            parser->desc_device = vsf_usbh_urb_get_buffer(urb);
        ...
        parse_next_config:
            ...
            err = vsf_usbh_get_descriptor(host, dev, USB_DT_CONFIG, dev->cur_config, 9);
            break;
        case VSF_USBH_PROBE_WAIT_CONFIG_DESC_SIZE:
            parser->desc_config = vsf_usbh_urb_get_buffer(urb);
            // get full configuation
            err = vsf_usbh_get_descriptor(host, dev, USB_DT_CONFIG,
        ...
        case VSF_USBH_PROBE_WAIT_FULL_CONFIG_DESC:
            ...
            parser->desc_config = vsf_usbh_urb_get_buffer(urb);
            err = vsf_usbh_set_configuration(host, dev, parser->desc_config->bConfigurationValue);
            break;
        case VSF_USBH_PROBE_WAIT_SET_CONFIG:
            if (vsf_usbh_parse_config(host, parser) < 0) {
                vsf_usbh_reset_parser(parser);
                dev->cur_config++;
    ...
    case VSF_EVT_TIMER:
        ...
        check_device_reset:
            if (vsf_usbh_hub_dev_is_reset(dev)) {
                ...
                err = vsf_usbh_set_address(host, dev);
            }
            break;
        case VSF_USBH_PROBE_WAIT_ADDRESS_STABLE:
            ...
            err = vsf_usbh_get_descriptor(host, dev, USB_DT_DEVICE, 0,
...
{
    vsf_usbh_t *host = container_of(eda, vsf_usbh_t, teda);
    vsf_err_t err = host->drv->init_evthandler(eda, evt, &host->use_as__vsf_usbh_hcd_t);

    ASSERT(err >= 0);
    if (VSF_ERR_NONE == err) {
        host->teda.evthandler = vsf_usbh_probe_evthandler;

#if VSF_USBH_CFG_ENABLE_ROOT_HUB == ENABLED
        host->dev_rh = vsf_usbh_alloc_device(host);
        ASSERT(host->dev_rh != NULL);

        host->dev_rh->speed = host->rh_speed;
        host->dev_new = host->dev_rh;
        vsf_eda_post_evt(eda, VSF_EVT_INIT);
#endif
    }
}

vsf_err_t vsf_usbh_init(vsf_usbh_t *host)
{
    ASSERT((host != NULL) && (host->drv != NULL));

    vsf_slist_init(&host->class_list);
    vsf_bitmap_reset(&host->device_bitmap, VSF_USBH_CFG_MAX_DEVICE);
    vsf_bitmap_set(&host->device_bitmap, 0);

    host->teda.evthandler = vsf_usbh_init_evthandler;
    return vsf_teda_init(&host->teda, vsf_priority_inherit, false);
}

vsf_err_t vsf_usbh_fini(vsf_usbh_t *host)
{
    // TODO
    return VSF_ERR_NONE;
}

void vsf_usbh_register_class(vsf_usbh_t *host, vsf_usbh_class_t *class)
{
    ASSERT((host != NULL) && (class != NULL));
    class->list.pnext = host->class_list.pnext;
    host->class_list.pnext = class;
}

vsf_err_t vsf_usbh_register_class_driver(vsf_usbh_t *host,
        const vsf_usbh_class_drv_t *drv)
{
    ASSERT((host != NULL) && (drv != NULL));
    vsf_usbh_class_t *class = VSF_USBH_MALLOC(sizeof(*class));
    if (class == NULL) { return VSF_ERR_FAIL; }

    class->drv = drv;
    vsf_usbh_register_class(host, class);
    return VSF_ERR_NONE;
}
