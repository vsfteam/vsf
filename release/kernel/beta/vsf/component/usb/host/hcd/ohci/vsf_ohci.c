...
static vsf_err_t ohci_ed_schedule(vsf_ohci_t *ohci, ohci_urb_t *urb_ohci)
{
    ...
    case USB_ENDPOINT_XFER_CONTROL:
        ...
        if (!ohci->ed_controltail && !ohci->ed_rm_list.pnext) {
    ...
    case USB_ENDPOINT_XFER_BULK:
        ...
        if (!ohci->ed_bulktail && !ohci->ed_rm_list.pnext)
        {
...
{
    ohci_ed_t *ed = urb_ohci->ed;
    ohci_td_t *td = urb_ohci->td_list.pnext, *td_next;

    while (td != NULL) {
        td_next = td->list.pnext;
        ohci_td_free(td);
        td = td_next;
    }
    urb_ohci->td_list.pnext = NULL;
    urb_ohci->length = 0;
        ...
        uint_fast16_t len, ohci_urb_t *urb_ohci)
{
    ...
    return (ohci_td_t *)td->list.pnext;
}
...
static void ohci_update_dl(vsf_ohci_t *ohci)
{
    ...
    while (td_next) {
        ...
        if ((cc != TD_CC_NOERROR) && (td->urb_ohci->ed->hwHeadP & ED_H)) {
            ...
            while (td != NULL) {
                td_tmp = td->list.pnext;
                if (td != ed->td_dummy) {
                    td->list.pnext = ohci->td_dl_list.pnext;
                    ohci->td_dl_list.pnext = td;
                }
                td = td_tmp;
            }
            continue;
...
static void ohci_finish_unlinks(vsf_ohci_t *ohci)
{
    ...
    for (ed_last = (ohci_ed_t **)&ohci->ed_rm_list.pnext, ed = *ed_last; ed != NULL; ed = *ed_last) {
        if (ed->rm_frame == frame) {
            ...
            ed_last = (ohci_ed_t **)&ed->list.pnext;
        }
    }
...
static vsf_err_t ohci_td_done(vsf_usbh_hcd_urb_t *urb, ohci_td_t *td)
{
    ...
    td->list.pnext = urb_ohci->td_list.pnext;
    urb_ohci->td_list.pnext = td;
...
static void ohci_ed_start_unlink(vsf_ohci_t *ohci, ohci_urb_t *urb_ohci)
{
    ...
    ed->list.pnext = ohci->ed_rm_list.pnext;
    ohci->ed_rm_list.pnext = ed;
    ed->ed_prev = NULL;
...
{
    ohci_td_t *td;
    while (ohci->td_dl_list.pnext != NULL) {
        td = (ohci_td_t *)ohci->td_dl_list.pnext;
        ohci->td_dl_list.pnext = td->list.pnext;
...
static void ohci_interrupt(void *param)
{
    ...
    if ((intrstatus & OHCI_INTR_SF) && !ohci->ed_rm_list.pnext) {
...
static void ohci_free_urb(vsf_usbh_hcd_t *hcd, vsf_usbh_hcd_urb_t *urb)
{
    ...
    if (urb_ohci->state) {
        ...
        ASSERT((urb_ohci->state & (URB_PRIV_EDLINK | URB_PRIV_TDALLOC)) ==
...
static vsf_err_t ohci_relink_urb(vsf_usbh_hcd_t *hcd, vsf_usbh_hcd_urb_t *urb)
{
    ...
    ASSERT((hcd != NULL) && (urb != NULL));
    ohci = (vsf_ohci_t *)hcd->priv;
    ASSERT((ohci != NULL) && ((ohci_urb_t *)urb->priv != NULL) &&
            (((ohci_urb_t *)urb->priv)->state == (URB_PRIV_EDLINK | URB_PRIV_TDALLOC)));
