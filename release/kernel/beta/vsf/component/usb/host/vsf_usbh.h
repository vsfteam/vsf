...
#include "utilities/ooc_class_simple.h"
...
#ifndef VSF_USBH_CFG_HEAP
#   define VSF_USBH_CFG_HEAP    VSF_HEAP
#endif
...
struct vsf_usbh_class_drv_t {
    ...
    void * (*probe)(vsf_usbh_t *host, vsf_usbh_dev_t *dev,
                    vsf_usbh_ifs_parser_t *parser_ifs);
    void (*disconnect)(vsf_usbh_t *host, vsf_usbh_dev_t *dev, void *param);
};
...
extern vsf_err_t vsf_usbh_init(vsf_usbh_t *host);
extern vsf_err_t vsf_usbh_fini(vsf_usbh_t *host);
extern vsf_err_t vsf_usbh_register_class_driver(vsf_usbh_t *host,
        const vsf_usbh_class_drv_t *drv);
extern void vsf_usbh_register_class(vsf_usbh_t *host, vsf_usbh_class_t *class);
#endif
...
void vsf_usbh_urb_prepare(vsf_usbh_urb_t *urb, vsf_usbh_dev_t *dev,
            struct usb_endpoint_desc_t *desc_ep);
extern vsf_err_t vsf_usbh_alloc_urb(vsf_usbh_t *host, vsf_usbh_urb_t *urb);
extern void vsf_usbh_free_urb(vsf_usbh_t *host, vsf_usbh_urb_t *urb);
...
extern void *vsf_usbh_urb_get_buffer(vsf_usbh_urb_t *urb);
extern void vsf_usbh_urb_set_buffer(vsf_usbh_urb_t *urb, void *buffer,
            uint_fast32_t size);
extern vsf_err_t vsf_usbh_urb_get_status(vsf_usbh_urb_t *urb);
extern vsf_err_t vsf_usbh_urb_get_actual_length(vsf_usbh_urb_t *urb);

extern vsf_err_t vsf_usbh_submit_urb(vsf_usbh_t *host, vsf_usbh_urb_t *urb);
extern vsf_err_t vsf_usbh_relink_urb(vsf_usbh_t *host, vsf_usbh_urb_t *urb);

#if defined(VSF_USBH_IMPLEMENT_HUB)
extern vsf_usbh_dev_t *vsf_usbh_alloc_device(vsf_usbh_t *host);
#endif

extern void vsf_usbh_disconnect_device(vsf_usbh_t *host, vsf_usbh_dev_t *dev);
extern void vsf_usbh_remove_interface(vsf_usbh_t *host, vsf_usbh_dev_t *dev,
        vsf_usbh_ifs_t *ifs);

extern vsf_err_t vsf_usbh_control_msg(vsf_usbh_t *host, vsf_usbh_dev_t *dev,
        struct usb_ctrlrequest_t *req);

extern vsf_err_t vsf_usbh_get_descriptor(vsf_usbh_t *host,
        vsf_usbh_dev_t *dev, uint_fast8_t type, uint_fast8_t index,
        uint_fast16_t size);
extern vsf_err_t vsf_usbh_get_class_descriptor(vsf_usbh_t *host,
        vsf_usbh_dev_t *dev, uint_fast16_t ifnum, uint_fast8_t type,
        uint_fast8_t id, uint_fast16_t size);
extern vsf_err_t vsf_usbh_set_Zuration(vsf_usbh_t *host,
        vsf_usbh_dev_t *dev, uint_fast8_t configuration);
extern vsf_err_t vsf_usbh_set_interface(vsf_usbh_t *host,
