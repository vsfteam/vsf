...
{
    vsf_usbh_dev_t *dev = hub->dev->children_list.pnext;
    do {
        if (dev->index == index) {
            return dev;
        }
        dev = dev->child_node.pnext;
    } while (dev != NULL);
    return NULL;
}

static vsf_err_t hub_set_port_feature(vsf_usbh_t *host, vsf_usbh_dev_t *dev,
...
static vsf_err_t hub_get_port_status(vsf_usbh_t *host, vsf_usbh_dev_t *dev,
        uint_fast16_t port)
{
    struct usb_ctrlrequest_t req = {
...
static vsf_err_t hub_clear_port_feature(vsf_usbh_t *host, vsf_usbh_dev_t *dev,
        uint_fast16_t port, uint_fast16_t feature)
{
    struct usb_ctrlrequest_t req = {
        .bRequestType    =  USB_TYPE_CLASS | USB_RECIP_OTHER | USB_DIR_OUT,
...
static vsf_err_t hub_get_status(vsf_usbh_t *host, vsf_usbh_dev_t *dev)
{
    struct usb_ctrlrequest_t req = {
...
static vsf_err_t hub_get_descriptor(vsf_usbh_t *host, vsf_usbh_dev_t *dev,
        uint_fast16_t size)
{
    struct usb_ctrlrequest_t req = {
        .bRequestType    =  USB_TYPE_CLASS | USB_RECIP_DEVICE | USB_DIR_IN,
...
static void vsf_usbh_hub_evthandler(vsf_eda_t *eda, vsf_evt_t evt)
{
    ...
    case VSF_EVT_INIT:
        ...
        err = hub_get_descriptor(hub->host, dev, 4);
        break;
    case VSF_EVT_MESSAGE:
        ...
        case HUB_STAT_ENUM_WAIT_HUB_DESC_LEN:
            ...
            err = hub_get_descriptor(hub->host, dev, hub->desc_hub.bDescLength);
            break;
        ...
        enum_set_next_power_on:
            hub->cur_dev_idx++;
            err = hub_set_port_feature(hub->host, dev, hub->cur_dev_idx,
                    USB_PORT_FEAT_POWER);
            break;
        ...
        scan_check_port_status:
            if (hub->hub_portsts.wPortChange & USB_PORT_STAT_C_CONNECTION) {
                ...
                err = hub_clear_port_feature(hub->host, dev, hub->cur_dev_idx,
        ...
        case HUB_STAT_CONNECT_WAIT_PORT_CLEAR_CONNECT_CHANGE:
            if (!(hub->hub_portsts.wPortStatus & USB_PORT_STAT_CONNECTION)) {
                ...
                if (dev != NULL) {
                    vsf_usbh_disconnect_device(hub->host, dev);
                }
    ...
    case VSF_EVT_TIMER:
        ...
        get_port_status:
            ...
            err = hub_get_port_status(hub->host, dev, hub->cur_dev_idx);
            break;
        ...
        case HUB_STAT_CONNECT_WAIT_PORT_RESET_STABLE:
            ...
            if (hub->host->dev_new != NULL) {
                vsf_teda_set_timer(200);
                retain_state = true;
            } else {
                vsf_usbh_dev_t *dev_new = vsf_usbh_alloc_device(hub->host);
                if (dev_new != NULL) {
                    dev_new->speed =
                    ...
                    dev_new->child_node.pnext = dev->children_list.pnext;
                    dev->children_list.pnext = dev_new;

                    hub->host->dev_new = dev_new;
                    vsf_eda_post_evt(&hub->host->teda.use_as__vsf_eda_t, VSF_EVT_INIT);
                }
...
fail:
    if (hub->state < HUB_STAT_ENUM_END) {
        vsf_usbh_remove_interface(hub->host, dev, hub->ifs);
    } else {
...
static void *vsf_usbh_hub_probe(vsf_usbh_t *host, vsf_usbh_dev_t *dev,
...
static void vsf_usbh_hub_disconnect(vsf_usbh_t *host, vsf_usbh_dev_t *dev,
        void *param)
{
