...
|vsf_pt_msg_queue_init|vsf_err_t vsf_pt_msg_queue_init(vsf_list_queue_t *pthis, uint_fast16_t max);|pthis|vsf_list_queue_t *|队列实例指针|vsf_list_queue_t类型的非空指针|队列初始化|
|||max|uint_fast16_t|队列长度|0~65535||
|vsf_pt_msg_enqueue|vsf_err_t vsf_pt_msg_enqueue(vsf_eda_queue_t *pthis, void *node, int_fast32_t timeout)|pthis|vsf_eda_queue_t *|队列实例指针|vsf_eda_queue_t类型的非空指针|入队|
|||node|void *|需要入队的指针|非空||
|||timeout|int_fast32_t|超时配置|0~2147483647||
|vsf_pt_msg_dequeue|vsf_err_t vsf_pt_msg_dequeue(vsf_eda_queue_t *pthis, void **node, int_fast32_t timeout)|pthis|vsf_eda_queue_t *|队列实例指针|vsf_eda_queue_t类型的非空指针|出队|
...
|||msg|void *|目标数据的指针|非空||
|vsf_pt_wfem|uintptr_t vsf_pt_wfem(vsf_evt_t evt);|evt|vsf_evt_t|事件|已定义的事件|等待指定的事件及获取事件携带的指针|
|vsf_eda_post_msg|vsf_err_t vsf_eda_post_msg(vsf_eda_t *pthis, void *msg)|pthis|vsf_eda_t *|任务实例指针|vsf_eda_t类型的非空指针|向目标任务发送一个指针|
|||msg|void *|目标数据的指针|非空||
|vsf_pt_wfm|uintptr_t vsf_pt_wfm(void)|无|无|无|无|获取一个事件携带指针|