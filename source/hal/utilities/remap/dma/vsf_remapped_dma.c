...
vsf_err_t vsf_remapped_dma_channel_acquire(vsf_remapped_dma_t *dma, vsf_dma_channel_hint_t *channel_hint_ptr)
{
    VSF_HAL_ASSERT((dma != NULL) && (dma->target != NULL));
    return vsf_dma_channel_acquire(dma->target, channel_hint_ptr);
}
