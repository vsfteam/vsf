...
#define ROM_FLASH_ERASE_ONE_SECTOR(__ADDR)      ((uint32_t (*)(uint32_t))0x1fff7c81)(__ADDR)
...
vsf_err_t VSF_MCONNECT(VSF_FLASH_CFG_IMP_PREFIX, _flash_erase_multi_sector)(
    ...
    offset += FLASH_BASE;
    for (vsf_flash_size_t i = 0; i < size; i += 4096) {
        if (0 != ROM_FLASH_ERASE_ONE_SECTOR(offset + i)) {
            return VSF_ERR_FAIL;
        }
    }

    vsf_flash_isr_t *isr_ptr = &flash_ptr->isr;
    if (    (flash_ptr->irq_mask & VSF_FLASH_IRQ_ERASE_MASK)
        &&  (isr_ptr->handler_fn != NULL)) {
        isr_ptr->handler_fn(isr_ptr->target_ptr, (vsf_flash_t *)flash_ptr, VSF_FLASH_IRQ_ERASE_MASK);
    }
    return VSF_ERR_NONE;
}
...
vsf_err_t VSF_MCONNECT(VSF_FLASH_CFG_IMP_PREFIX, _flash_write_multi_sector)(
    ...
    vsf_err_t result = (0 == ROM_FLASH_WRITE(offset + FLASH_BASE, buffer, size)) ? VSF_ERR_NONE : VSF_ERR_FAIL;

    vsf_flash_isr_t *isr_ptr = &flash_ptr->isr;
    if (    (flash_ptr->irq_mask & VSF_FLASH_IRQ_WRITE_MASK)
        &&  (isr_ptr->handler_fn != NULL)) {
        isr_ptr->handler_fn(isr_ptr->target_ptr, (vsf_flash_t *)flash_ptr, VSF_FLASH_IRQ_WRITE_MASK);
    }
    return result;
}
...
vsf_err_t VSF_MCONNECT(VSF_FLASH_CFG_IMP_PREFIX, _flash_read_multi_sector)(
    ...
    memcpy(buffer, (void *)(offset + FLASH_BASE), size);
