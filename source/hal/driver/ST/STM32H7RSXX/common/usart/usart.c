...
vsf_err_t VSF_MCONNECT(VSF_USART_CFG_IMP_PREFIX, _usart_init)(
    ...
    ctl |= (cfg_ptr->mode & __VSF_HW_USART_MODE_CR1_MASK) | USART_CR1_FIFOEN;
    reg->CR1 = ctl | over8;

    ctl = reg->CR2;
    ctl &= ~(USART_CR2_CLKEN | USART_CR2_CPHA | USART_CR2_CPOL | USART_CR2_STOP | USART_CR2_SWAP | USART_CR2_RXINV | USART_CR2_TXINV);
    cfg = cfg_ptr->mode & __VSF_HW_USART_MODE_CR2_MASK;
    ctl |= (((cfg & 0x03) << 12) | ((cfg & 0x180) << 2) | (cfg & ~0x183));
    reg->CR2 = ctl;

    ctl = reg->CR3;
    ctl &= ~(USART_CR3_RTSE | USART_CR3_CTSE | USART_CR3_HDSEL);
    cfg = cfg_ptr->mode & __VSF_HW_USART_MODE_CR3_MASK;
...
void VSF_MCONNECT(VSF_USART_CFG_IMP_PREFIX, _usart_irq_enable)(
    ...
    usart_ptr->reg->CR1 |= irq_mask & __VSF_HW_USART_IRQ_CR1_MASK;
    usart_ptr->reg->CR3 |= irq_mask & __VSF_HW_USART_IRQ_CR3_MASK;
}
