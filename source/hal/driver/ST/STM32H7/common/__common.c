...
#define __STM32H7_SWI(__N, __VALUE)                                             \
    ROOT ISR(SWI##__N##_IRQHandler)                                             \
    {                                                                           \
        if (__stm32h7_common.swi[__N].handler != NULL) {                        \
            __stm32h7_common.swi[__N].handler(__stm32h7_common.swi[__N].param); \
        }                                                                       \
    }
...
static const IRQn_Type __stm32h7_soft_irq[VSF_DEV_SWI_NUM] = {
    VSF_DEV_SWI_LIST
};

typedef struct stm32h7_common_t {
    struct {
...
#if __VSF_DEV_SWI_NUM > 0
static stm32h7_common_t __stm32h7_common;
#endif
...
// SWI
#if __VSF_DEV_SWI_NUM > 0
VSF_MREPEAT(__VSF_DEV_SWI_NUM, __STM32H7_SWI, NULL)
                                                ...
                                                void *param)
{
    if (idx < __VSF_DEV_SWI_NUM) {
        if (handler != NULL) {
            __stm32h7_common.swi[idx].handler = handler;
            __stm32h7_common.swi[idx].param = param;

            NVIC_SetPriority(__stm32h7_soft_irq[idx], priority);
            NVIC_EnableIRQ(__stm32h7_soft_irq[idx]);
        } else {
            NVIC_DisableIRQ(__stm32h7_soft_irq[idx]);
        }
        return VSF_ERR_NONE;
    }
...
{
    if (idx < __VSF_DEV_SWI_NUM) {
        NVIC_SetPendingIRQ(__stm32h7_soft_irq[idx]);
        return;
    }
    VSF_HAL_ASSERT(false);
}
