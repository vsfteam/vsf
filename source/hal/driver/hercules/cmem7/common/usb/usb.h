...
#ifndef __HAL_DRIVER_HERCULES_CMEM7_USB_H__
#define __HAL_DRIVER_HERCULES_CMEM7_USB_H__
...
#define ____USB_OTG_DEF(__N, __VALUE)                                           \
    extern cmem7_usb_t USB_OTG##__N##_IP;                                       \
...
typedef struct cmem7_usb_const_t {
    uint8_t dc_ep_num;
    uint8_t hc_ep_num;
    void *reg;
...
typedef struct cmem7_usb_t {
    bool is_host;
    struct {
        usb_ip_irqhandler_t irqhandler;
        void *param;
    } callback;
    const cmem7_usb_const_t *param;
} cmem7_usb_t;
