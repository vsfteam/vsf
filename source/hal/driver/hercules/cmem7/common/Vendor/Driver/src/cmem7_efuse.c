...
void EFUSE_DeInit(void) {
		SOFT_RESET->SOFTRST_b.EFUSE_n = 0;
		SOFT_RESET->SOFTRST_b.EFUSE_n = 1;
}
...
void EFUSE_StructInit(EFUSE_InitTypeDef *init) {
  assert_param(init);
  
	/* UART_InitStruct members default value */
	init->EFUSE_ClockDividor = 8;
	init->EFUSE_TMRF = EFUSE_TMRF_R_1200;
  if (init->timing) {
    init->timing->EFUSE_Tpwph = 480;
    init->timing->EFUSE_Trac = 33;
		init->timing->EFUSE_Trah = 9;
		init->timing->EFUSE_Trpw = 33;
		init->timing->EFUSE_Trc = 66;
		init->timing->EFUSE_Tesr = 9;
		init->timing->EFUSE_Tprs = 480;
		init->timing->EFUSE_Tpi = 800;
		init->timing->EFUSE_Tpp = 10000;
		init->timing->EFUSE_Teps = 53;
		init->timing->EFUSE_Tpwps = 480;
  }
}
