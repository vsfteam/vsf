...
static uint32_t i2c_GetClock(I2C0_Type* I2Cx, BOOL LP) {
    uint32_t dividor = 0;
    ...
    return LP ? SYSTEM_CLOCK_FREQ_LP / (1 << (dividor + 1)) : SYSTEM_CLOCK_FREQ / (1 << (dividor + 1));
}
...
void I2C_DeInit(I2C0_Type* I2Cx) {
    assert_param(IS_I2C_ALL_PERIPH(I2Cx));

    if (I2Cx == I2C0){
        SOFT_RESET->SOFTRST_b.I2C0_n = 0;
        SOFT_RESET->SOFTRST_b.I2C0_n = 1;
    } else {
        SOFT_RESET->SOFTRST_b.I2C1_n = 0;
        SOFT_RESET->SOFTRST_b.I2C1_n = 1;
    }
}

void I2C_Init(I2C0_Type* I2Cx, I2C_InitTypeDef* I2C_Init, BOOL LP) {
    assert_param(IS_I2C_ALL_PERIPH(I2Cx));
    ...
    if (I2C_Init->timing) {
        ...
        value = i2c_GetClock(I2Cx,LP) / I2C_Init->timing->I2C_Freq / 2;
        CMEM7_BFI(&(I2Cx->SCL_CNT), value, 0, 16);
        CMEM7_BFI(&(I2Cx->SCL_CNT), value, 16, 16);
        I2Cx->SDA_SETUP_b.TSU_DAT = ((uint64_t)I2C_Init->timing->I2C_TsuDat) *
            i2c_GetClock(I2Cx,LP) / 1000000000;
...
void I2C_StructInit(I2C_InitTypeDef* init) {
    ...
    init->I2C_Mode = I2C_Mode_Master;
    init->I2C_AddressWidth = I2C_ADDR_WIDTH_7BIT;
    if (init->timing) {
        init->timing->I2C_Freq = 400000;
        init->timing->I2C_TsuDat = 100;
        init->timing->I2C_Tsetup = 600;
        init->timing->I2C_Tbuf = 1300;
        init->timing->I2C_TsuSta = 600;
        init->timing->I2C_SdaFilterEn = FALSE;
        init->timing->I2C_SdaFilterSpike = 0;
        init->timing->I2C_SclFilterEn = FALSE;
        init->timing->I2C_SclFilterSpike = 0;
    }
}

void I2C_Cmd(I2C0_Type* I2Cx, BOOL enable) {
    assert_param(IS_I2C_ALL_PERIPH(I2Cx));

    I2Cx->ENABLE_b.EN = enable;
}

void I2C_ITConfig(I2C0_Type* I2Cx, uint32_t Int, BOOL enable) {
    assert_param(IS_I2C_ALL_PERIPH(I2Cx));
...
BOOL I2C_GetITStatus(I2C0_Type* I2Cx, uint32_t Int) {
    assert_param(IS_I2C_ALL_PERIPH(I2Cx));
    assert_param(IS_I2C_INT(Int));
...
void I2C_ClearITPendingBit(I2C0_Type* I2Cx, uint32_t Int) {
    assert_param(IS_I2C_ALL_PERIPH(I2Cx));
    assert_param(IS_I2C_INT(Int));
...
BOOL I2C_GetFlagStatus(I2C0_Type* I2Cx, uint32_t Flag) {
    assert_param(IS_I2C_ALL_PERIPH(I2Cx));
    assert_param(IS_I2C_FLAG(Flag));

    if (0 != (I2Cx->STATUS & Flag)) {
        return TRUE;
    }
...
void I2C_ClearFlag(I2C0_Type* I2Cx, uint32_t Flag) {
    assert_param(IS_I2C_ALL_PERIPH(I2Cx));
    assert_param(IS_I2C_FLAG(Flag));

    if (Flag & I2C_Flag_RX_FIFO_NOT_EMPTY) {
        // It can't be clear by sw but read
    }

    if (Flag & I2C_Flag_RD_REQUEST) {
        i2c_ReadClear(I2Cx->CLR_RD_REQ_b.CLEAR);
    }

    if (Flag & I2C_Flag_TX_ABORT) {
        i2c_ReadClear(I2Cx->CLR_TX_ABRT_b.CLEAR);
    }

    if (Flag & I2C_Flag_RX_DONE) {
        i2c_ReadClear(I2Cx->CLR_RX_DONE_b.CLEAR);
    }

    if (Flag & I2C_Flag_TX_DONE) {
        i2c_ReadClear(I2Cx->CLR_TX_DONE_b.CLEAR);
    }
}

BOOL I2C_MasterReadReq(I2C0_Type* I2Cx, uint16_t Addr, uint8_t size) {
    assert_param(IS_I2C_ALL_PERIPH(I2Cx));
    ...
    if (I2Cx->CTRL_b.MODE == I2C_Mode_Master) {
        I2Cx->TAR_b.ADDR10 = i2c_NormalizeAddr(I2Cx, Addr);
    }
    if (I2Cx->CTRL_b.MODE == I2C_Mode_Slave) {
        I2Cx->SAR_b.ADDR10 = i2c_NormalizeAddr(I2Cx, Addr);
    }
    ...
    if (size != 0) {
        ...
        __set_PRIMASK(irq);//__enable_irq();
    }
...
BOOL I2C_WriteReq(I2C0_Type* I2Cx, uint8_t size, uint16_t Addr, uint8_t firstData) {
    assert_param(IS_I2C_ALL_PERIPH(I2Cx));
