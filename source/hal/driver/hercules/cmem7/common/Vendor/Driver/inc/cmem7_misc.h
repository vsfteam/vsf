...
#define CMEM7_BFI(addr, value, lsb, len)            \
  do {                                              \
    ...
    unsigned long tmp, mask;                        \
                                                    \
    alignedAddr = (unsigned long)addr & 0xFFFFFFFC; \
    mask = (0xFFFFFFFF >> lsb) << (32 - len);       \
    mask >>= (32 - len - lsb);                      \
    tmp = *(volatile unsigned long *)alignedAddr;   \
    tmp &= ~mask;                                   \
    tmp |= (((unsigned long)value) << lsb);         \
    *(volatile unsigned long *)alignedAddr = tmp;   \
  } while (0)
