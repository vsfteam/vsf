...
void GPIO_DeInit(void) {
  SOFT_RESET->SOFTRST_b.GPIO_n = 0;
	SOFT_RESET->SOFTRST_b.GPIO_n = 1;
}
...
void GPIO_ITConfig(uint8_t Group, uint32_t Enable) {
	assert_param(IS_GPIO_GROUP(Group));
...
uint32_t GPIO_GetITStatus(uint8_t Group) {
	assert_param(IS_GPIO_GROUP(Group));
...
void GPIO_ClearITPendingBit(uint8_t Group, uint32_t Clear) {
	assert_param(IS_GPIO_GROUP(Group));
...
void GPIO_InitPwm(uint8_t Channel, uint32_t HighLevelNanoSecond, uint32_t LowLevelNanoSecond) {
    ...
    CMEM7_BFI(&(GPIO->PWM_OUT0_LEN), lowTick, 0, 16);
    CMEM7_BFI(&(GPIO->PWM_OUT0_LEN), highTick, 16, 16);
	} else if (Channel == GPIO_PWM_CHANNEL_GPIO_H_9) {
		CMEM7_BFI(&(GPIO->PWM_OUT1_LEN), lowTick, 0, 16);
    CMEM7_BFI(&(GPIO->PWM_OUT1_LEN), highTick, 16, 16);
	} else if (Channel == GPIO_PWM_CHANNEL_GPIO_H_19) {
		CMEM7_BFI(&(GPIO->PWM_OUT2_LEN), lowTick, 0, 16);
    CMEM7_BFI(&(GPIO->PWM_OUT2_LEN), highTick, 16, 16);
  } else {
		CMEM7_BFI(&(GPIO->PWM_OUT3_LEN), lowTick, 0, 16);
    CMEM7_BFI(&(GPIO->PWM_OUT3_LEN), highTick, 16, 16);
	}
}
