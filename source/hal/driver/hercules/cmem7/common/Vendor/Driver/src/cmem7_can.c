...
void CAN_Deinit(CAN0_Type* CANx) {
  ...
  if (CANx == CAN0){
    SOFT_RESET->SOFTRST_b.CAN0_n = 0;
    SOFT_RESET->SOFTRST_b.CAN0_n = 1;
  } else if (CANx == CAN1) {
    SOFT_RESET->SOFTRST_b.CAN1_n = 0;
    SOFT_RESET->SOFTRST_b.CAN1_n = 1;
  }
}
...
void CAN_StructInit(CAN_InitTypeDef *init) {
  ...
  init->CAN_TxEn = TRUE;
  init->CAN_Loopback = FALSE;
  init->CAN_ClockDiv = CAN_CDR_DIV_1_8;
  init->CAN_Prescaler = 0;
  init->CAN_SJW = 0;
  init->CAN_TSEG1 = 5;
  init->CAN_TSEG2 = 2;
  init->CAN_HighSpeed = FALSE;
}
...
BOOL CAN_GetFlagStatus(CAN0_Type* CANx, uint32_t flag) {
  assert_param(IS_CAN_ALL_PERIPH(CANx));
  assert_param(IS_CAN_FLAG(flag));
  
  if (0 != (CANx->STATUS & flag)) {
		return TRUE;
	}
