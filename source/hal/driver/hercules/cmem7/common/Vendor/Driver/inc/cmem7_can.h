...
/** @defgroup CAN_FLAG
  * @{
  */
#define CAN_FLAG_RBF               0x01
#define CAN_FLAG_DATA_OVERRUN      0x02
#define CAN_FLAG_TB_UNLOCK         0x04
#define CAN_FLAG_TX_DONE           0x08
#define CAN_FLAG_RECEIVING         0x10
#define CAN_FLAG_TRANSMITTING      0x20
#define CAN_FLAG_ERR               0x40
#define CAN_FLAG_BUS_ERR           0x80
#define CAN_FLAG_All               0xFF

#define IS_CAN_FLAG(FLAG)          (((FLAG) != 0) && (((FLAG) & ~CAN_FLAG_All) == 0))                                  
/**
  * @}
  */
...
typedef struct {
  ...
  BOOL      RTR;                   /*!< remote frame flag                            */
	uint8_t   data1;                 /*!< data byte 1, reserved in 2nd filter whose 
                                        type is CAN_FLT_STANDARD_DUAL                */
	uint8_t   data2;                 /*!< data byte 2, valid in filter whose type
                                        is CAN_FLT_STANDARD_SINGLE                   */
} CAN_STANDARD_FILTER;
...
typedef struct {
	...
	uint16_t  ID12_0;                /*!< 13 bits, valid in filter whose type is
                                        CAN_FLT_EXTENDED_SINGLE                      */
  BOOL      RTR;                   /*!< remote frame flag, valid in filter whose 
                                        type is CAN_FLT_EXTENDED_SINGLE              */
} CAN_EXTENDED_FILTER;
...
/**
  ...
  * @retval None
	*/ 
void CAN_Deinit(CAN0_Type* CANx);
...
/**
  * @brief  Fills each CAN_InitTypeDef member with its default value.
  * @param[in] init A pointer to structure CAN_InitTypeDef which will be initialized.
  * @retval None
  */ 
...
/**
  ...
  * @retval None
	*/ 
void CAN_ITConfig(CAN0_Type* CANx, uint32_t Int, BOOL enable);
...
/**
  * @brief  Check specific interrupts are set or not 
	...
	*/
uint8_t CAN_GetITStatus(CAN0_Type* CANx);

/**
  * @brief  Check specific flags are set or not 
	* @param[in] CANx CAN peripheral, which is CAN0 or CAN1
  * @param[in] uint8_t CAN flags bits, which can be the combination of @ref CAN_FLAG
	* @retval BOOL The bit indicates if specific flags are set or not
	*/
BOOL CAN_GetFlagStatus(CAN0_Type* CANx, uint32_t flag);
