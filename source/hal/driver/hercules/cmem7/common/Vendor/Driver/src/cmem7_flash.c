...
static void flash_setClock(uint8_t dividor) {
	...
	dividor = (dividor >> 1) - 1;
  CMEM7_BFI(&(NOR_FLASH->CTRL0), dividor, 0, 8);
}
...
static uint8_t flash_ReadInnerStatusLow() {
  CMEM7_BFI(&(NOR_FLASH->CTRL0), 1, 16, 16);
  CMEM7_BFI(&(NOR_FLASH->CTRL1), FLASH_CMD_RD_INNER_STATUS_LOW, 0, 8);
...
static uint8_t flash_ReadInnerStatusHigh() {
  CMEM7_BFI(&(NOR_FLASH->CTRL0), 1, 16, 16);
  CMEM7_BFI(&(NOR_FLASH->CTRL1), FLASH_CMD_RD_INNER_STATUS_HIGH, 0, 8);
	NOR_FLASH->TRIGGER_b.OP_START = TRUE;
...
static void flash_WriteWriteEnable(BOOL enable) {
  ...
  CMEM7_BFI(&(NOR_FLASH->CTRL0), 0, 16, 16);  
  value = enable ? FLASH_CMD_WR_WRITE_ENABLE : 
    FLASH_CMD_WR_WRITE_DISABLE;
	CMEM7_BFI(&(NOR_FLASH->CTRL1), value, 0, 8);
	NOR_FLASH->TRIGGER_b.OP_START = TRUE;
...
static void flash_WriteStatusReg(FLASH_INNER_STATUS *s) {
  uint16_t tmp = s->INNER.STATUS;
	...
	CMEM7_BFI(&(NOR_FLASH->CTRL0), 2, 16, 16);
	CMEM7_BFI(&(NOR_FLASH->CTRL1), FLASH_CME_WR_STATUS_REG, 0, 8);
...
static void flash_Erase(uint8_t cmd, uint32_t addr) {	
	CMEM7_BFI(&(NOR_FLASH->CTRL0), 0, 16, 16);
	CMEM7_BFI(&(NOR_FLASH->CTRL1), cmd, 0, 8);
...
static void flash_WriteDeepPowerDownEnable(BOOL enable) {
	...
	CMEM7_BFI(&(NOR_FLASH->CTRL0), 0, 16, 16);
	value = enable ? FLASH_CME_WR_ENTER_DEEP_PD : 
		FLASH_CME_WR_EXIT_DEEP_PD;
	CMEM7_BFI(&(NOR_FLASH->CTRL1), value, 0, 8);
...
static void flash_RwReq(uint8_t cmd, uint32_t addr, uint16_t size) {
	CMEM7_BFI(&(NOR_FLASH->CTRL0), size, 16, 16);
	CMEM7_BFI(&(NOR_FLASH->CTRL1), cmd, 0, 8);
	NOR_FLASH->CTRL1_b.ADDRESS = addr;
...
static void flash_RdIdReq() {
	CMEM7_BFI(&(NOR_FLASH->CTRL0), 0, 16, 16);
	CMEM7_BFI(&(NOR_FLASH->CTRL1), FLASH_RD_ID, 0, 8);
...
void FLASH_Deinit(void) {
  SOFT_RESET->SOFTRST_b.FLS_n = 0;
  SOFT_RESET->SOFTRST_b.FLS_n = 1;
}
...
void FLASH_StructInit(FLASH_InitTypeDef *init) {
  ...
  init->FLASH_ClockDividor = 8;
  init->FLASH_QuadEnable = FALSE;
  init->FLASH_Wait = 0; 
}

void FLASH_GetId(uint8_t *manufacturer, uint16_t *id) {
  uint8_t data[3], count = 0;
  ...
  while (count < 3) {
	  ...
	  count += flash_ReadFifo(3 - count, &data[count]);
	}
