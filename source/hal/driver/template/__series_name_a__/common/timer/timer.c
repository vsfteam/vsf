...
/*\note For IPCore drivers, define __VSF_HAL_${TIMER_IP}_TIMER_CLASS_IMPLEMENT before including vsf_hal.h.
 *      For peripheral drivers, if IPCore driver is used, define __VSF_HAL_${TIMER_IP}_TIMER_CLASS_INHERIT__ before including vsf_hal.h
 */
...
/*\note VSF_HW_TIMER_CFG_MULTI_CLASS is only for drivers for specified device(hw drivers).
 *      For other drivers, please define VSF_${TIMER_IP}_TIMER_CFG_MULTI_CLASS in header file.
 */
...
/*\note Implementation below is for hw timer only, because there is no requirements
...
void VSF_MCONNECT(VSF_TIMER_IRQ_CFG_IMP_PREFIX, _timer_irq_enable)(
    VSF_MCONNECT(VSF_TIMER_CFG_IMP_PREFIX, _timer_t) *timer_ptr,
...
void VSF_MCONNECT(VSF_TIMER_IRQ_CFG_IMP_PREFIX, _timer_irq_disable)(
    VSF_MCONNECT(VSF_TIMER_CFG_IMP_PREFIX, _timer_t) *timer_ptr,
    vsf_timer_irq_mask_t irq_mask
) {
    VSF_HAL_ASSERT(timer_ptr != NULL);
...
vsf_err_t VSF_MCONNECT(VSF_TIMER_PWM_CFG_IMP_PREFIX, _timer_pwm_set)(
    VSF_MCONNECT(VSF_TIMER_CFG_IMP_PREFIX, _timer_t) *timer_ptr,
    uint8_t channel,
    uint32_t period,
    uint32_t pulse
...
#define VSF_TIMER_CFG_IMP_LV0(__IDX, __HAL_OP)                                    \
    VSF_MCONNECT(VSF_TIMER_CFG_IMP_PREFIX, _timer_t)                                \
        ...
        .reg                = VSF_MCONNECT(VSF_TIMER_CFG_IMP_UPCASE_PREFIX, _TIMER, __IDX,_REG_),\
        __HAL_OP                                                                \
    };                                                                          \
    VSF_CAL_ROOT void VSF_MCONNECT(VSF_TIMER_CFG_IMP_UPCASE_PREFIX, _TIMER, __IDX, _IRQHandler)(void)\
    {                                                                           \
