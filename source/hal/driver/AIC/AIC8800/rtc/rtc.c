...
vsf_err_t vsf_hw_rtc_get(vsf_hw_rtc_t *hw_rtc_ptr, vsf_rtc_tm_t *rtc_tm)
{
    ...
    time_t time = vsf_hw_rtc_get_second(hw_rtc_ptr);

    struct tm tm_local;
    gmtime_offset_r(&time, &tm_local, 0);
...
vsf_err_t vsf_hw_rtc_set(vsf_hw_rtc_t *hw_rtc_ptr, const vsf_rtc_tm_t *rtc_tm)
{
    ...
    return vsf_hw_rtc_set_second(hw_rtc_ptr, time);
}
...
vsf_err_t vsf_hw_rtc_set_second(vsf_hw_rtc_t *hw_rtc_ptr, time_t time)
{
    aic_time_update((uint32_t)time, 0);
    return VSF_ERR_NONE;
}
