 ...
 * VSF_WDT_CFG_REIMPLEMENT_TYPE_CTRL to redefine enum
 * @ref vsf_wdt_ctrl_t.
 * \~chinese
 * @brief 在特定硬件驱动中，启用宏 VSF_WDT_CFG_REIMPLEMENT_TYPE_CTRL
 * 来重新定义枚举 @ref vsf_wdt_ctrl_t。
 */
#ifndef VSF_WDT_CFG_REIMPLEMENT_TYPE_CTRL
#   define VSF_WDT_CFG_REIMPLEMENT_TYPE_CTRL        DISABLED
#endif
...
#define VSF_WDT_APIS(__prefix_name) \
    ...
    __VSF_HAL_TEMPLATE_API(__prefix_name, vsf_err_t,            wdt, ctrl,        VSF_MCONNECT(__prefix_name, _t) *wdt_ptr, vsf_wdt_ctrl_t ctrl, void* param)
...
#if VSF_WDT_CFG_REIMPLEMENT_TYPE_CTRL == DISABLED
/**
 * \~english
 * @brief Predefined VSF WDT control commands that can be reimplemented in specific HAL drivers.
 * @note Used as the 'ctrl' parameter (second parameter) in vsf_wdt_ctrl(vsf_wdt_t *wdt_ptr, vsf_wdt_ctrl_t ctrl, void *param) function
 * @note The 'param' parameter (third parameter) type depends on the specific control command
 * \~chinese
 * @brief 预定义的 VSF WDT 控制命令，可以在特定的 HAL 驱动中重新实现。
 * @note 作为 vsf_wdt_ctrl(vsf_wdt_t *wdt_ptr, vsf_wdt_ctrl_t ctrl, void *param) 函数中的 'ctrl' 参数（第二个参数）
 ...
 * - Mandatory: None (only __VSF_WDT_CTRL_DUMMY is defined by default)
 ...
 *
 * 功能分类：
 * - 必选：无（默认仅定义 __VSF_WDT_CTRL_DUMMY）
 * - 标准可选：无（未定义标准可选功能）
 */
typedef enum vsf_wdt_ctrl_t {
    ...
    //!       (VSF_WDT_CFG_REIMPLEMENT_TYPE_CTRL == DISABLED) and all optional control
    ...
    //!       redefine the enum in a specific hardware driver (VSF_WDT_CFG_REIMPLEMENT_TYPE_CTRL == ENABLED),
    ...
    //!       (VSF_WDT_CFG_REIMPLEMENT_TYPE_CTRL == DISABLED) 且所有可选控制命令都被注释掉时。
    //!       它确保枚举至少有一个成员，以避免某些不允许空枚举的 C 编译器报错。
    //! @note 如果启用了以下任何控制命令（取消注释），或者在特定硬件驱动中重新定义了枚举
    //!       (VSF_WDT_CFG_REIMPLEMENT_TYPE_CTRL == ENABLED)，只要定义了至少一个实际命令，
    //!       就可以删除此 DUMMY 值。
...
extern vsf_err_t vsf_wdt_ctrl(vsf_wdt_t *wdt_ptr, vsf_wdt_ctrl_t ctrl, void * param);
...
#   define vsf_wdt_ctrl(__WDT, ...)         VSF_MCONNECT(VSF_WDT_CFG_PREFIX, _wdt_ctrl)                     ((__vsf_wdt_t *)(__WDT), ##__VA_ARGS__)
#endif
/// @endcond
