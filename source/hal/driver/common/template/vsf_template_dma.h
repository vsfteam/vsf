...
#define VSF_DMA_APIS(__prefix_name) \
    ...
    __VSF_HAL_TEMPLATE_API(__prefix_name, int8_t,                   dma, channel_request,      VSF_MCONNECT(__prefix_name, _dma_t) *dma_ptr, void *filter_param) \
...
typedef void vsf_dma_isr_handler_t(void *target_ptr, vsf_dma_t *dma_ptr, vsf_dma_irq_mask_t irq_mask);
 ...
 */
extern int8_t vsf_dma_channel_request(vsf_dma_t *dma_ptr, void *filter_param);
...
/**
 ...
 @param[in] dma_ptr: 结构体 vsf_dma_t 的指针，参考 @ref vsf_dma_t
 @param[in] channel: 通道序号
 @return vsf_err_t: 如果 dma 取消成功返回 VSF_ERR_NONE , 否则返回负数。
 */
