 ...
 * Contains the current status of TIMER operations and FIFO thresholds
 * \~chinese
 * @brief TIMER 状态信息结构体
 * 包含 TIMER 操作的当前状态和 FIFO 阈值
 */
...
typedef struct vsf_timer_status_t {
    union {
        ...
        struct {
            uint32_t is_busy         : 1;    //!< \~english TIMER is busy with any operation \~chinese TIMER 正在进行任何操作
        };
        uint32_t value;
    };
} vsf_timer_status_t;
