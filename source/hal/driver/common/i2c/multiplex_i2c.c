...
static vsf_err_t __m_i2c_request_immediately(vsf_multiplexer_i2c_t *multiplexer);
...
static vsf_err_t __m_i2c_request_immediately(vsf_multiplexer_i2c_t *multiplexer)
{
    ...
    vsf_multiplex_i2c_t *m_i2c_ptr = multiplexer->req_m_i2c;
    if (m_i2c_ptr != multiplexer->inited_m_i2c) {
...
static void __i2c_start_next(vsf_multiplexer_i2c_t *multiplexer)
{
    ...
    if (!vsf_slist_queue_is_empty(&multiplexer->slist_queue)) {
        vsf_slist_queue_dequeue(vsf_multiplex_i2c_t, slist_node, &multiplexer->slist_queue, multiplexer->req_m_i2c);
        vsf_err_t result = __m_i2c_request_immediately(multiplexer);
        VSF_HAL_ASSERT(result == VSF_ERR_NONE);
        (void)result;
    }
}
...
static void __i2c_isr_handler(void *target_ptr, vsf_i2c_t *i2c_ptr, vsf_i2c_irq_mask_t irq_mask)
{
    ...
    if ((irq_mask & cpl_irq_mask) && is_stop) {
        VSF_HAL_ASSERT((m_i2c_ptr->request.cmd & VSF_I2C_CMD_RESTART) == 0);
        multiplexer->req_m_i2c = NULL;
...
vsf_err_t vsf_multiplex_i2c_init(vsf_multiplex_i2c_t *m_i2c_ptr, vsf_i2c_cfg_t *cfg_ptr)
{
    ...
    vsf_protect_t state = vsf_multiplex_i2c_protect();
        if (   (multiplexer->inited_m_i2c == NULL)              // first init
            || (multiplexer->inited_m_i2c == m_i2c_ptr)) {      // re-init
                                           ...
                                           uint8_t *buffer_ptr)
{
    ...
    vsf_protect_t state = vsf_multiplex_i2c_protect();
        if (vsf_slist_queue_is_in(vsf_multiplex_i2c_t, slist_node, &multiplexer->slist_queue, m_i2c_ptr)) {
            ...
            VSF_HAL_ASSERT(0);
            result = VSF_ERR_FAIL;
        } else if (multiplexer->req_m_i2c == NULL) {
            if ((cmd & VSF_I2C_CMD_START) == 0) { // first request must include VSF_I2C_CMD_START
                VSF_HAL_ASSERT(0);
                result = VSF_ERR_FAIL;
            } else {
                multiplexer->req_m_i2c = m_i2c_ptr;
                result = __m_i2c_request_immediately(multiplexer);
            }
        } else if (multiplexer->req_m_i2c == m_i2c_ptr) {
            result = __m_i2c_request_immediately(multiplexer);
        } else {
