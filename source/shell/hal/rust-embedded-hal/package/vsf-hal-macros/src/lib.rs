...
pub fn bind_vsf_peripherials(_item: TokenStream) -> TokenStream {
    ...
    bind_vsf_gpios(&bindings_lines, &mut output_code);
    bind_vsf_peripheral(&bindings_lines, "usart", &mut output_code);
    ...
    for index in 0..32 {
        if mask & (1 << index) != 0 {
            ...
            if let Some(mut pin_mask) = extract_const_integer::<u32>(&lines, &format!("VSF_HW_GPIO_PORT{index}_MASK")) {
                ...
                while pin_mask != 0 {
                if pin_mask & 1 != 0 {
                    output_code.push_str(&format!("#[cfg(VSF_HW_GPIO_PORT{index}_PIN{pin_index})]P{port_ch}{pin_index},"));
                }
                pin_index += 1;
                pin_mask >>= 1;
                }
...
fn bind_vsf_peripheral(lines: &Vec<&str>, name: &str, output_code: &mut String) {
    let peripheral_name_lower = String::from(name).to_lowercase();
    let peripheral_name_upper = String::from(name).to_uppercase();
    let mask = extrace_peripheral_mask(lines, &peripheral_name_upper);

    for index in 0..32 {
        if mask & (1 << index) != 0 {
            output_code.push_str(&format!("#[cfg(vsf_{peripheral_name_lower}{index}_enabled)]{peripheral_name_upper}{index},"));
        }
    }
}
