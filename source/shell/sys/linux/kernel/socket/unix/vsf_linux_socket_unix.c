...
static int __vsf_linux_socket_unix_connect(vsf_linux_socket_priv_t *socket_priv, const struct sockaddr *addr, socklen_t addrlen)
{
    ...
    vsf_queue_stream_t *queue_stream = (vsf_queue_stream_t *)vsf_linux_get_rx_stream(priv->rw.sfd_tx);
...
static int __vsf_linux_socket_unix_accept(vsf_linux_socket_priv_t *socket_priv, struct sockaddr *addr, socklen_t *addr_len)
{
    ...
    wait_next:
        ...
        fail:
            ...
            vsf_queue_stream_t *queue_stream = (vsf_queue_stream_t *)vsf_linux_get_rx_stream(priv_remote->rw.sfd_rx);
            vsf_linux_fd_t *sfd_tx = vsf_linux_tx_pipe(queue_stream);
            if (NULL == sfd_tx) {
