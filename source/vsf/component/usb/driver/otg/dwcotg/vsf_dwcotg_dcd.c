...
static void vk_dwcotg_usbd_flush_txfifo(vk_dwcotg_dcd_t *usbd, uint_fast8_t fifo_num)
{
...
static void vk_dwcotg_usbd_flush_rxfifo(vk_dwcotg_dcd_t *usbd)
{
    usbd->reg.global_regs->grstctl = USB_OTG_GRSTCTL_RXFFLSH;
    while (usbd->reg.global_regs->grstctl & USB_OTG_GRSTCTL_RXFFLSH);
}

vsf_err_t vk_dwcotg_usbd_init(vk_dwcotg_dcd_t *usbd, usb_dc_cfg_t *cfg)
{
    ...
    const vk_dwcotg_dcd_param_t *param = usbd->param;
    ...
    vk_dwcotg_usbd_init_regs(usbd, info.regbase, info.ep_num);
...

    {
        usb_dc_ip_cfg_t ip_cfg = {
            ...
            .irq_handler    = (usb_ip_irq_handler_t)vk_dwcotg_usbd_irq,
            .param          = usbd,
        };
...
void vk_dwcotg_usbd_fini(vk_dwcotg_dcd_t *usbd)
{
    usbd->param->op->Fini();
}

void vk_dwcotg_usbd_reset(vk_dwcotg_dcd_t *usbd, usb_dc_cfg_t *cfg)
{
...
void vk_dwcotg_usbd_connect(vk_dwcotg_dcd_t *usbd)
{
    debug_add_evt(__FUNC_CONNECT);
    usbd->reg.dev.global_regs->dctl &= ~USB_OTG_DCTL_SDIS;
}

void vk_dwcotg_usbd_disconnect(vk_dwcotg_dcd_t *usbd)
{
    debug_add_evt(__FUNC_DISCONNECT);
    usbd->reg.dev.global_regs->dctl |= USB_OTG_DCTL_SDIS;
}

void vk_dwcotg_usbd_wakeup(vk_dwcotg_dcd_t *usbd)
{
}

void vk_dwcotg_usbd_set_address(vk_dwcotg_dcd_t *usbd, uint_fast8_t addr)
{

}

uint_fast8_t vk_dwcotg_usbd_get_address(vk_dwcotg_dcd_t *usbd)
{
    return (usbd->reg.dev.global_regs->dcfg & USB_OTG_DCFG_DAD) >> 4;
}

uint_fast16_t vk_dwcotg_usbd_get_frame_number(vk_dwcotg_dcd_t *usbd)
{
...
void vk_dwcotg_usbd_get_setup(vk_dwcotg_dcd_t *usbd, uint8_t *buffer)
{
    memcpy(buffer, &usbd->setup, sizeof(usbd->setup));
}

void vk_dwcotg_usbd_status_stage(vk_dwcotg_dcd_t *usbd, bool is_in)
{
    debug_add_evt(__FUNC_STATUS_STAGE);

    usbd->ctrl_transfer_state = DWCOTG_STATUS_STAGE;
    if (is_in) {
        vk_dwcotg_usbd_ep_transfer_send(usbd, 0x80, NULL, 0, true);
    } else {
        vk_dwcotg_usbd_ep_transfer_recv(usbd, 0, NULL, 0);
    }
}
...
static volatile uint32_t * dwcotg_usbd_get_ep_ctrl(vk_dwcotg_dcd_t *usbd, uint_fast8_t ep)
{
...
vsf_err_t vk_dwcotg_usbd_ep_add(vk_dwcotg_dcd_t *usbd, uint_fast8_t ep, usb_ep_type_t type, uint_fast16_t size)
{
    ...
    *ep_ctrl &= ~USB_OTG_DIEPCTL_MPSIZ;
    ...
    if (is_in) {
        ...
        vk_dwcotg_usbd_flush_txfifo(usbd, 0x10);
        vk_dwcotg_usbd_flush_rxfifo(usbd);
#endif
    }
...
uint_fast16_t vk_dwcotg_usbd_ep_get_size(vk_dwcotg_dcd_t *usbd, uint_fast8_t ep)
{
    volatile uint32_t *ep_ctrl = dwcotg_usbd_get_ep_ctrl(usbd, ep);
...
vsf_err_t vk_dwcotg_usbd_ep_transaction_read_buffer(vk_dwcotg_dcd_t *usbd, uint_fast8_t ep, uint8_t *buffer, uint_fast16_t size)
{
    VSF_USB_ASSERT(false);
    return VSF_ERR_NOT_SUPPORT;
}

vsf_err_t vk_dwcotg_usbd_ep_transaction_enable_out(vk_dwcotg_dcd_t *usbd, uint_fast8_t ep)
{
    VSF_USB_ASSERT(false);
    return VSF_ERR_NOT_SUPPORT;
}

vsf_err_t vk_dwcotg_usbd_ep_transaction_set_data_size(vk_dwcotg_dcd_t *usbd, uint_fast8_t ep, uint_fast16_t size)
{
    VSF_USB_ASSERT(false);
    return VSF_ERR_NOT_SUPPORT;
}

vsf_err_t vk_dwcotg_usbd_ep_transaction_write_buffer(vk_dwcotg_dcd_t *usbd, uint_fast8_t ep, uint8_t *buffer, uint_fast16_t size)
{
    VSF_USB_ASSERT(false);
    return VSF_ERR_NOT_SUPPORT;
}

static vk_dwcotg_dcd_trans_t * vk_dwcotg_usbd_get_trans(vk_dwcotg_dcd_t *usbd, uint_fast8_t ep)
{
    uint_fast8_t is_in = ep & 0x80;
    ep &= 0x0F;
...
static void vk_dwcotg_usbd_ep_write(vk_dwcotg_dcd_t *usbd, uint_fast8_t ep_idx)
{
    debug_add_evt(__FUNC_EP_WRITE);

    vk_dwcotg_dcd_trans_t *trans = vk_dwcotg_usbd_get_trans(usbd, ep_idx | 0x80);
    if (!trans->use_dma) {
        struct dwcotg_dev_global_regs_t *dev_global_regs = usbd->reg.dev.global_regs;
...
static void vk_dwcotg_usbd_ep_read(vk_dwcotg_dcd_t *usbd, uint_fast8_t ep_idx, uint_fast16_t size)
{
    debug_add_evt(__FUNC_EP_READ);

    vk_dwcotg_dcd_trans_t *trans = vk_dwcotg_usbd_get_trans(usbd, ep_idx);
    if (!trans->use_dma) {
        uint_fast16_t ep_size = vk_dwcotg_usbd_ep_get_size(usbd, ep_idx);
        uint8_t *buffer = trans->buffer;
        uint32_t data;
...
uint_fast32_t vk_dwcotg_usbd_ep_get_data_size(vk_dwcotg_dcd_t *usbd, uint_fast8_t ep)
{
    vk_dwcotg_dcd_trans_t *trans = vk_dwcotg_usbd_get_trans(usbd, ep);
    return trans->size - trans->remain;
}

static vsf_err_t vk_dwcotg_usbd_ep_out_transfer(vk_dwcotg_dcd_t *usbd, uint_fast8_t ep_idx)
{
...
vsf_err_t vk_dwcotg_usbd_ep_transfer_recv(vk_dwcotg_dcd_t *usbd, uint_fast8_t ep, uint8_t *buffer, uint_fast32_t size)
{
    ...
    vk_dwcotg_dcd_trans_t *trans = &usbd->trans[ep];
    trans->buffer = buffer;
    trans->remain = size;
    trans->size = size;
    trans->zlp = false;
    ...
    return vk_dwcotg_usbd_ep_out_transfer(usbd, ep);
}

static vsf_err_t vk_dwcotg_usbd_ep_in_transfer(vk_dwcotg_dcd_t *usbd, uint_fast8_t ep_idx)
{
    debug_add_evt(__FUNC_EP_IN_TRANSFER);
    VSF_USB_ASSERT(ep_idx < usbd->ep_num);

    uint_fast16_t ep_size = vk_dwcotg_usbd_ep_get_size(usbd, ep_idx | 0x80);
    struct dwcotg_dev_in_ep_regs_t *in_regs = &usbd->reg.dev.ep.in_regs[ep_idx];
    vk_dwcotg_dcd_trans_t *trans = &usbd->trans[VSF_DWCOTG_DCD_CFG_EP_NUM + ep_idx];
    ...
    if (trans->use_dma) {
        ...
        in_regs->diepctl |= USB_OTG_DIEPCTL_EPENA | USB_OTG_DIEPCTL_CNAK;
        vk_dwcotg_usbd_ep_write(usbd, ep_idx);
    }
    return VSF_ERR_NONE;
}

vsf_err_t vk_dwcotg_usbd_ep_transfer_send(vk_dwcotg_dcd_t *usbd, uint_fast8_t ep, uint8_t *buffer, uint_fast32_t size, bool zlp)
{
    ...
    vk_dwcotg_dcd_trans_t *trans = &usbd->trans[VSF_DWCOTG_DCD_CFG_EP_NUM + ep];
    trans->buffer = buffer;
    trans->remain = size;
    trans->size = size;
    trans->zlp = zlp;
    ...
    return vk_dwcotg_usbd_ep_in_transfer(usbd, ep);
}

static void vk_dwcotg_usbd_notify(vk_dwcotg_dcd_t *usbd, usb_evt_t evt, uint_fast8_t value)
{
...
void vk_dwcotg_usbd_irq(vk_dwcotg_dcd_t *usbd)
{
    ...
    if (intsts & USB_OTG_GINTSTS_USBRST) {
        ...
        vk_dwcotg_usbd_notify(usbd, USB_ON_RESET, 0);
            ...
            if (ep_int & 0x1) {
                ...
                if (int_status & USB_OTG_DIEPINT_XFRC) {
                    if ((ep_idx == 0) && (usbd->ctrl_transfer_state == DWCOTG_STATUS_STAGE)) {
                        ...
                        vk_dwcotg_usbd_notify(usbd, USB_ON_STATUS, 0);
                    } else {
                        trans = &usbd->trans[VSF_DWCOTG_DCD_CFG_EP_NUM + ep_idx];
                        if (trans->remain) {
                            debug_add_evt(__USB_ON_ISR_IN_TRANSFER);
                            vk_dwcotg_usbd_ep_in_transfer(usbd, ep_idx);
                        } else {
                            debug_add_evt(__USB_ON_IN);
                            vk_dwcotg_usbd_notify(usbd, USB_ON_IN, ep_idx);
                        }
                    }
                ...
                if (int_status & USB_OTG_DOEPINT_XFRC) {
                    if ((ep_idx == 0) && (usbd->ctrl_transfer_state == DWCOTG_STATUS_STAGE)) {
                        ...
                        if (!trans->zlp) {
                            ...
                            vk_dwcotg_usbd_ep_out_transfer(usbd, ep_idx);
                        } else {
                            debug_add_evt(__USB_ON_OUT);
                            vk_dwcotg_usbd_notify(usbd, USB_ON_OUT, ep_idx);
                        }
                    }
    ...
    if (intsts & USB_OTG_GINTSTS_RXFLVL) {
        ...
        case 6: //RXSTAT_SETUP_UPDT:
            if (!ep_idx && (8 == size) && (0/*DPID_DATA0*/ == pid)) {
                ...
                ((uint32_t *)usbd->setup)[0] = *usbd->use_as__vk_dwcotg_t.reg.dfifo[0];
                ((uint32_t *)usbd->setup)[1] = *usbd->use_as__vk_dwcotg_t.reg.dfifo[0];
        ...
        case 2: //RXSTAT_DATA_UPDT:
            ...
            vk_dwcotg_usbd_ep_read(usbd, ep_idx, size);
            break;
