...
{
    class_internal(item_ptr, this_ptr, vsf_pbuf_t);
    VSF_SERVICE_ASSERT(NULL != this_ptr);
    const vsf_pbuf_adapter_t *adapter_ptr = NULL;
    
    do {
        uint_fast8_t id = this.u8_adapter_id;
        ...
        adapter_ptr = &__vsf_pbuf_cb.adapters_ptr[id];
...
{
    VSF_SERVICE_ASSERT(NULL != (void *)target_ptr);
    vsf_pbuf_cfg_t cfg = {
