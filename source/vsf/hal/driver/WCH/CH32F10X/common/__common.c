...
static const IRQn_Type __ch32f10x_soft_irq[VSF_DEV_SWI_NUM] = {
    VSF_DEV_SWI_LIST
};

struct __ch32f10x_common_t {
    struct {
        vsf_swi_handler_t *handler;
        void *param;
    } swi[__VSF_DEV_SWI_NUM];
};
typedef struct __ch32f10x_common_t __ch32f10x_common_t;
#endif
...
#if __VSF_DEV_SWI_NUM > 0
static __ch32f10x_common_t __ch32f10x_common;
#endif
...
// SWI
#if __VSF_DEV_SWI_NUM > 0
REPEAT_MACRO(__VSF_DEV_SWI_NUM, __CH32F10X_SWI, NULL)
                                                ...
                                                void *param)
{
    if (idx < __VSF_DEV_SWI_NUM) {
        if (handler != NULL) {
            __ch32f10x_common.swi[idx].handler = handler;
            __ch32f10x_common.swi[idx].param = param;

            NVIC_SetPriority(__ch32f10x_soft_irq[idx], priority);
            NVIC_EnableIRQ(__ch32f10x_soft_irq[idx]);
        } else {
            NVIC_DisableIRQ(__ch32f10x_soft_irq[idx]);
        }
        return VSF_ERR_NONE;
    }
...
{
    if (idx < __VSF_DEV_SWI_NUM) {
        NVIC_SetPendingIRQ(__ch32f10x_soft_irq[idx]);
        return;
    }
    VSF_HAL_ASSERT(false);
}
