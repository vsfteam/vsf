...
#define __STM32F103_SWI(__N, __VALUE)                                           \
    ROOT ISR(SWI##__N##_IRQHandler)                                             \
    {                                                                           \
        if (__stm32f103_common.swi[__N].handler != NULL) {                      \
            __stm32f103_common.swi[__N].handler(__stm32f103_common.swi[__N].param);\
        }                                                                       \
    }
...
static const IRQn_Type __stm32f103_soft_irq[VSF_DEV_SWI_NUM] = {
    VSF_DEV_SWI_LIST
};

struct __stm32f103_common_t {
    struct {
        vsf_swi_handler_t *handler;
        void *param;
    } swi[__VSF_DEV_SWI_NUM];
};
typedef struct __stm32f103_common_t __stm32f103_common_t;
#endif
...
#if __VSF_DEV_SWI_NUM > 0
static __stm32f103_common_t __stm32f103_common;
#endif
...
// SWI
#if __VSF_DEV_SWI_NUM > 0
REPEAT_MACRO(__VSF_DEV_SWI_NUM, __STM32F103_SWI, NULL)
                                                ...
                                                void *param)
{
    if (idx < __VSF_DEV_SWI_NUM) {
        if (handler != NULL) {
            __stm32f103_common.swi[idx].handler = handler;
            __stm32f103_common.swi[idx].param = param;

            NVIC_SetPriority(__stm32f103_soft_irq[idx], priority);
            NVIC_EnableIRQ(__stm32f103_soft_irq[idx]);
        } else {
            NVIC_DisableIRQ(__stm32f103_soft_irq[idx]);
        }
        return VSF_ERR_NONE;
    }
...
{
    if (idx < __VSF_DEV_SWI_NUM) {
        NVIC_SetPendingIRQ(__stm32f103_soft_irq[idx]);
        return;
    }
    VSF_HAL_ASSERT(false);
}
