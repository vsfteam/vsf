...
#ifdef VSF_USBH_BTHCI_CFG_PRIORITY
    vsf_eda_init(&bthci->eda, VSF_USBH_BTHCI_CFG_PRIORITY);
#else
...
vsf_err_t vk_usbh_bthci_send(void *dev, uint8_t type, uint8_t *packet, uint16_t size)
{
    ...
    case BTHCI_PACKET_TYPE_CMD:
        if (ocb->is_ep0_claimed || __vsf_eda_crit_npb_enter(&bthci->dev->ep0.crit, 0)) {
            return VSF_ERR_NOT_AVAILABLE;
        } else {
