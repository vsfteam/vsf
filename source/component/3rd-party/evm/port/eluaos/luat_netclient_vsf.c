...
static int32_t netclient_thread_init(netclient_t *thiz)
{
    ...
    pthread_t thread;
    if (0 > pthread_create(&thread, NULL, netclient_thread_entry, thiz))
    {
    ...
    while (1)
    {
        ...
        if (FD_ISSET(thiz->sock_fd, &fds))
        {
            ...
            else
            {
                ...
                res = recvfrom(thiz->sock_fd, sock_buff, BUFF_SIZE, 0, (struct sockaddr *)&from, &addrlen);
            }
