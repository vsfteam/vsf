/*****************************************************************************
 *   Copyright(C)2009-2022 by SimonQian                                      *
 *                                                                           *
...
#define __VSFIP_NETIF_CLASS_IMPLEMENT
#include "../vsfip.h"
...
enum {
    VSFIP_NETIF_EVT_ARPC_UPDATED    = (VSF_EVT_USER + 0),
    VSFIP_NETIF_EVT_OUTPUTTED       = (VSF_EVT_USER + 1),
};

typedef enum vsfip_netif_arp_op_t {
    ARP_REQUEST     = 1,
    ARP_REPLY       = 2,
    RARP_REQUEST    = 3,
    RARP_REPLY      = 4,
} vsfip_netif_arp_op_t;

typedef struct vsfip_arp_head_t {
    uint16_t hwtype;
    uint16_t prototype;
    uint8_t hwlen;
    uint8_t protolen;
...
static void __vsfip_netif_destruct(vsfip_netif_t *netif)
{
    vsf_eda_fini(&netif->arpc.teda.use_as__vsf_eda_t);
    vsf_eda_sync_cancel(&netif->arpc.sem);
    vsf_eda_sync_cancel(&netif->output_sem);
}

static vsf_err_t __vsfip_netif_construct(vsfip_netif_t *netif)
{
...
{
    vk_netdrv_fini(netif->netdrv);
}

uint_fast16_t vsfip_netif_mtu(vsfip_netif_t *netif)
{
    vk_netdrv_t *netdrv = netif->netdrv;
    if (netdrv != NULL) {
        return netdrv->mtu;
    }
    return 0;
...
//    so __vsfip_netif_construct is called protected
void vsfip_netif_ref(vsfip_netif_t *netif)
{
    if (!netif->ref++) {
        __vsfip_netif_construct(netif);
    }
#ifdef VSFIP_TRACE_NETIF
    __vsfip_netif_trace_ref(netif);
#endif
}
...
void vsfip_netif_deref(vsfip_netif_t *netif)
{
    ...
    if (!netif->ref) {
        __vsfip_netif_destruct(netif);
        netif->netdrv = NULL;
    }
}

static void __vsfip_netif_get_mac_broadcast(vsfip_netif_t *netif, vsfip_macaddr_t *macaddr)
{
...
static bool __vsfip_netif_get_mac(vsfip_netif_t *netif, vsfip_ipaddr_t *ip, vsfip_macaddr_t *mac)
{
    if (0xFFFFFFFF == ip->addr32) {
        __vsfip_netif_get_mac_broadcast(netif, mac);
        return true;
    }
...
static vsf_err_t __vsfip_netif_output(vsfip_netbuf_t *netbuf,
...
static void __vsfip_netif_get_ipaddr(vsfip_netbuf_t *netbuf, vsfip_ipaddr_t *ipaddr)
{
    ...
    if (4 == ipver) {
        // IPv4
        vsfip_ip4_head_t *ip4_head = netbuf->buf.obj_ptr;
        ipaddr->size = 4;
...
static bool __vsfip_netif_islocal(vsfip_netif_t *netif, vsfip_ipaddr_t *ipaddr)
{
...
vsf_err_t vsfip_netif_ip_output(vsfip_netbuf_t *netbuf, bool urgent)
{
    ...
    __vsfip_netif_get_ipaddr(netbuf, &dest);
    ip_for_mac = __vsfip_netif_islocal(netif, &dest) || !netif->gateway.size ?
                    &dest : &netif->gateway;
    if (!__vsfip_netif_get_mac(netif, ip_for_mac, &mac) && netif->netdrv->macaddr.size) {
        vsf_sched_lock_status_t origlevel = vsf_sched_lock();
        ...
        uint_fast8_t hwlen, uint8_t *hwaddr, uint_fast8_t protolen, uint8_t *protoaddr)
{
    ...
    if (__vsfip_netif_get_mac(netif, &ip, &mac)) {
        return;
    }
...
{
    vsfip_netif_t *netif = netbuf->netif;
    vsfip_arp_head_t *head = netbuf->buf.obj_ptr;
...
static vsfip_netbuf_t * __vsfip_netif_prepare_arp_request(
...
static void __vsfip_netif_arpc_evthandler(vsf_eda_t *eda, vsf_evt_t evt)
{
    ...
    case VSF_EVT_SYNC:
        ...
        __vsfip_netif_get_ipaddr(netif->arpc.cur_netbuf, &dest);
        ...
        netif->arpc.ip_for_mac = __vsfip_netif_islocal(netif, &dest) || !netif->gateway.size ? dest : netif->gateway;
        if (__vsfip_netif_get_mac(netif, &netif->arpc.ip_for_mac, &mac)) {
            goto mac_got;
        }

        netif->arpc.retry = VSFIP_CFG_ARP_RETRY;
    retry:
        netif->arpc.cur_request = __vsfip_netif_prepare_arp_request(netif, &netif->arpc.ip_for_mac);
        if (NULL == netif->arpc.cur_request) {
            goto failed;
        }

        __vsfip_netif_get_mac_broadcast(netif, &mac);
        if (VSF_ERR_NONE != __vsfip_netif_output(netif->arpc.cur_request, VSFIP_NETIF_PROTO_ARP, &mac, false)) {
            goto failed;
        }
    ...
    case VSFIP_NETIF_EVT_ARPC_UPDATED:
        ...
        //         and if proxy ARP is enabled on router, router will reply
        netif->arpc.ip_for_mac = __vsfip_netif_islocal(netif, &dest) || !netif->gateway.size ? dest : netif->gateway;
        if (!__vsfip_netif_get_mac(netif, &netif->arpc.ip_for_mac, &mac) && (netif->arpc.retry > 0)) {
            netif->arpc.retry--;
            goto retry;
        }
    ...
    mac_got:
        if (!mac.size) {
            ...
            __vsfip_netif_output(netif->arpc.cur_netbuf, VSFIP_NETIF_PROTO_IP, &mac, false);
        }
        goto wait_next_request;
    }
}
