"""gpio_irq_latency suite host harness.
...
def decode(adapter: LogicAnalyzer, channels: dict, capture_path: Path,
    ...
    scenario = params.get("gpio_irq_latency", {})
    ...
    try:
        ...
        print(f"[SKIP] gpio_irq_latency decode: no LA channel for '{la_channel}'")
        return

    windows = read_framework_windows(adapter, channels, capture_path, "gpio_irq_latency",
    ...
    for case in cases:
        ...
        print(f"[PASS] CASE {idx}  gpio_irq_latency  edges={len(edges)}")
