"""gpio_open_drain suite host harness.
...
def decode(adapter: LogicAnalyzer, channels: dict, capture_path: Path,
    ...
    scenario = params.get("gpio_open_drain", {})
    ...
    try:
        ...
        print(f"[SKIP] gpio_open_drain decode: no LA channel for '{la_channel}'")
        return

    windows = read_framework_windows(adapter, channels, capture_path, "gpio_open_drain",
    ...
    for case in cases:
        ...
        print(f"[PASS] CASE {idx}  gpio_open_drain  edges={len(edges)}")
