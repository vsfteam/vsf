...
Usage:
    vsf-bench-test board/pico/hardware-map.yml \\
        --script vsf.demo/vsf/test/vsf_test/gpio/suite/vsf_test_gpio_output_input.py \\
...
def decode(adapter: LogicAnalyzer, channels: dict, capture_path: Path,
    ...
    try:
        ...
        print(f"[SKIP] gpio_output_input decode: no LA channel for '{la_channel}'")
        return

    windows = read_framework_windows(adapter, channels, capture_path, "gpio_output_input",
    ...
    for case in cases:
        ...
        print(f"[PASS] CASE {idx}  gpio_output_input  edges={len(edges)}")
