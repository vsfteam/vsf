"""gpio_multi_pin suite host harness.
...
def decode(adapter: LogicAnalyzer, channels: dict, capture_path: Path,
    ...
    try:
        ...
        print(f"[SKIP] gpio_multi_pin decode: no LA channel for '{la_channel}'")
        return

    windows = read_framework_windows(adapter, channels, capture_path, "gpio_multi_pin",
    ...
    for case in cases:
        ...
        print(f"[PASS] CASE {idx}  gpio_multi_pin  edges={len(edges)}")
