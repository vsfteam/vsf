...
const struct vsf_test_i2c_eeprom_rw_s {
    vsf_test_suite_t hdr;
    vsf_test_i2c_eeprom_rw_params_t          params[VSF_TEST_I2C_EEPROM_RW_CASE_COUNT];
} vsf_test_i2c_eeprom_rw = {
    .hdr = {
        .name            = "i2c_eeprom_rw",
        .jmp_fn          = vsf_test_i2c_eeprom_rw_run,
        .case_count      = VSF_TEST_I2C_EEPROM_RW_CASE_COUNT,
        .params_stride   = sizeof(vsf_test_i2c_eeprom_rw_params_t),
        .peripheral_type = VSF_PERIPHERAL_TYPE_I2C,
        .needs_ready     = false,
    },
    .params = { VSF_TEST_I2C_EEPROM_RW_PARAMS_INIT },
};
#endif
                      ...
                      vsf_i2c_irq_mask_t irq_mask)
{
    ...
    vsf_test_suite_data.i2c.i2c_eeprom_rw.irq_mask |= irq_mask;
}
...
{
    while (timeout_ms-- > 0) {
        if (vsf_test_suite_data.i2c.i2c_eeprom_rw.irq_mask & VSF_I2C_IRQ_MASK_MASTER_TRANSFER_COMPLETE) {
            return I2C_POLL_COMPLETE;
        }
        if (vsf_test_suite_data.i2c.i2c_eeprom_rw.irq_mask & VSF_I2C_IRQ_MASK_MASTER_ERR) {
            return I2C_POLL_ERR;
        }
...
static bool __i2c_wait_complete(const vsf_test_suite_t *suite, uint32_t timeout_ms)
{
    ...
    if (result != I2C_POLL_COMPLETE) {
        VSF_TEST_TRACE_ERROR("i2c_eeprom_rw:__i2c_wait_complete failed (result=%d) (irq_mask=0x%x)"
                             VSF_TRACE_CFG_LINEEND, (int)result,
                             (unsigned)vsf_test_suite_data.i2c.i2c_eeprom_rw.irq_mask);
        return false;
    }
    return true;
}
...
{
    while (max_ms-- > 0) {
        vsf_test_suite_data.i2c.i2c_eeprom_rw.irq_mask = 0;
        ...
        VSF_TEST_ASSERT_ERR_NONE(err,
            "i2c_eeprom_rw:__eeprom_ack_poll:vsf_i2c_master_request failed (err=%d) (addr=0x%02x)"
        ...
        if (result == I2C_POLL_TIMEOUT) {
            VSF_TEST_TRACE_ERROR("i2c_eeprom_rw:__eeprom_ack_poll timeout (addr=0x%02x) (irq_mask=0x%x)"
...
{
    vsf_test_i2c_eeprom_rw_params_t *p = tc->params;
    VSF_TEST_ASSERT_INST(inst, VSF_PERIPHERAL_TYPE_I2C, i2c);
    vsf_i2c_t *i2c = inst->fixture.i2c;
    uint8_t data_len = p->data_len;

    if (data_len == 0) {
        VSF_TEST_TRACE_ERROR("i2c_eeprom_rw:data_len is 0 (data_len=%u)" VSF_TRACE_CFG_LINEEND, (unsigned)data_len);
    }
    VSF_TEST_ASSERT(data_len > 0);
    VSF_TEST_ASSERT(data_len <= VSF_TEST_I2C_CASE_MAX_COUNT);

    memset(vsf_test_suite_data.i2c.i2c_eeprom_rw.write_buf, 0, sizeof(vsf_test_suite_data.i2c.i2c_eeprom_rw.write_buf));
    memset(vsf_test_suite_data.i2c.i2c_eeprom_rw.read_buf, 0, sizeof(vsf_test_suite_data.i2c.i2c_eeprom_rw.read_buf));
    vsf_test_suite_data.i2c.i2c_eeprom_rw.irq_mask = 0;
    ...
    vsf_err_t err = vsf_i2c_init(i2c, &(vsf_i2c_cfg_t){
        ...
        .isr        = {
            ...
            .target_ptr = suite,
            .prio       = VSF_TEST_I2C_EEPROM_RW_PRIO,
        },
    });
    VSF_TEST_ASSERT_ERR_NONE(err,
        "i2c_eeprom_rw:vsf_i2c_init failed (err=%d) (mode=0x%x clock_hz=%lu)"
    ...
    vsf_test_suite_data.i2c.i2c_eeprom_rw.write_buf[0] = p->mem_addr;
    for (uint8_t i = 0; i < data_len; i++) {
        vsf_test_suite_data.i2c.i2c_eeprom_rw.write_buf[1 + i] = (uint8_t)(VSF_TEST_I2C_EEPROM_RW_PATTERN_BYTE + i);
    }
    ...
    err = vsf_i2c_master_request(i2c, p->eeprom_addr,
        VSF_I2C_CMD_START | VSF_I2C_CMD_STOP | VSF_I2C_CMD_WRITE | VSF_I2C_CMD_7_BITS,
        data_len + 1, vsf_test_suite_data.i2c.i2c_eeprom_rw.write_buf);
    VSF_TEST_ASSERT_ERR_NONE(err,
        "i2c_eeprom_rw:vsf_i2c_master_request(write) failed (err=%d) (addr=0x%02x count=%u)"
        VSF_TRACE_CFG_LINEEND, (int)err,
    ...
    VSF_TEST_ASSERT(__eeprom_ack_poll(suite, i2c, p->eeprom_addr,
                                      &vsf_test_suite_data.i2c.i2c_eeprom_rw.write_buf[0], VSF_TEST_I2C_EEPROM_ACK_POLL_MAX_MS));
    ...
    err = vsf_i2c_master_request(i2c, p->eeprom_addr,
        ...
        1, &vsf_test_suite_data.i2c.i2c_eeprom_rw.write_buf[0]);
    VSF_TEST_ASSERT_ERR_NONE(err,
        "i2c_eeprom_rw:vsf_i2c_master_request(addr) failed (err=%d) (addr=0x%02x)"
    ...
    err = vsf_i2c_master_request(i2c, p->eeprom_addr,
        ...
        data_len, vsf_test_suite_data.i2c.i2c_eeprom_rw.read_buf);
    VSF_TEST_ASSERT_ERR_NONE(err,
        "i2c_eeprom_rw:vsf_i2c_master_request(read) failed (err=%d) (addr=0x%02x count=%u)"
        VSF_TRACE_CFG_LINEEND, (int)err,
    ...
    for (uint8_t i = 0; i < data_len; i++) {
        if (vsf_test_suite_data.i2c.i2c_eeprom_rw.read_buf[i] != vsf_test_suite_data.i2c.i2c_eeprom_rw.write_buf[1 + i]) {
            VSF_TEST_TRACE_ERROR("i2c_eeprom_rw:data mismatch at offset=%u (expected=0x%02x actual=0x%02x)"
                                 VSF_TRACE_CFG_LINEEND,
                                 (unsigned)i,
                                 vsf_test_suite_data.i2c.i2c_eeprom_rw.write_buf[1 + i],
                                 vsf_test_suite_data.i2c.i2c_eeprom_rw.read_buf[i]);
            data_match = false;
            break;
        }
    }
    ...
    VSF_TEST_TRACE_INFO("i2c_eeprom_rw:pass len=%u" VSF_TRACE_CFG_LINEEND,
                   (unsigned)data_len);
}
