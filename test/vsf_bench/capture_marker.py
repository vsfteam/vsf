...
from vsf_bench.utils import read_csv_rows
...
# Cache decoded marker channel data so multiple suites share a single
# dsview-cli call.  Key: (capture_path, channel, baudrate, start_ns, end_ns)
_marker_cache: dict = {}


def _find_markers(
    text: str, timestamps: list[int], pattern: str,
) -> list[MarkerEvent]:
...
def _read_marker_channel(
    ...
    key = (str(capture_path), channel, marker_baud, decode_start_ns, decode_end_ns)
    if key in _marker_cache:
        return _marker_cache[key]

    cfg = UARTConfig(baudrate=marker_baud)
    csv_path = adapter.decode_uart(capture_path, channel, cfg, decode_start_ns, decode_end_ns)
...
def read_framework_windows(
    ...
    marker_channel: str = "uart0_tx",
) -> list[CaseWindow]:
    ch = channels.get(marker_channel)
    if ch is None:
        raise KeyError(f"Marker channel role '{marker_channel}' not in channels: {list(channels)}")
    text, timestamps = _read_marker_channel(
        adapter, capture_path, ch, marker_baud, decode_start_ns, decode_end_ns,
    )

    starts = _find_markers(text, timestamps, rf"{suite_name}:case:(\d+)(?![\d:])")
    readys = _find_markers(text, timestamps, rf"{suite_name}:case:(\d+):ready")
    dones = _find_markers(text, timestamps, rf"{suite_name}:case:(\d+):done")
    ends = _find_markers(text, timestamps, rf"{suite_name}:end")
    ...
    for i, ev in enumerate(starts_sorted):
        ...
        if use_ready:
            if case_idx not in ready_by_case:
                raise RuntimeError(f"{suite_name}: case {case_idx} missing READY marker")
            if case_idx not in done_by_case:
                raise RuntimeError(f"{suite_name}: case {case_idx} has READY but no DONE marker")
